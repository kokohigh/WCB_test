...
for log in event_logs:
    if log['topics'][0].hex() == event_signature:
        addr= log['data'].hex()
        #print(addr)
        new_contract_address = w3.to_checksum_address(addr[24:64])
        print(f"New vote deployed at: {new_contract_address}")
        print("Vote start at:", int(addr[65:128],16))
        print("Vote finish at:", int(addr[129:192],16))
        break